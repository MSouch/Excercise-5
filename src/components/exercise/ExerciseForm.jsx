...
const ExerciseForm=({exercise,onComplete})=> {
  ...
  useEffect(()=> {
    // Auto-save every 30 seconds while actively filling the form (not after completion or when viewing model answer)
    const interval=setInterval(()=> {
      if (!completed && !showModelAnswer && Object.keys(formData).length > 0) {
        handleAutoSave()
      }
    },30000)
    return ()=> clearInterval(interval)
  },[formData, completed, showModelAnswer])
  ...
  const handleAutoSave=()=> {
    ...
    // Show saved banner only during active editing state
    if (!completed && !showModelAnswer) {
      setSaved(true)
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      >
        ...
        <div className="p-6">
          {saved && !showModelAnswer && !completed && (
            <motion.div
