...
const AuthGuard = ({ children }) => {
  ...
  if (location.pathname === '/conclusion' && progress.completedChallenges < 4) {
    // Allow if navigation explicitly requested conclusion (defensive nav from dashboard)
    const allow = location.state?.allowConclusion
    if (!allow) {
      console.log(`AuthGuard: Conclusion blocked - only ${progress.completedChallenges}/4 challenges completed`)
      return <Navigate to="/dashboard" replace />
    }
    console.log('AuthGuard: allowConclusion flag present, permitting /conclusion')
  }
